...
class CPandasFile:
    ...
    def readCsv(self, path: str, usecols=None, index_col=None):
        """ Đọc file csv

        Args:
            path ([str]): đường dẫn đến file cần đọc, có thể là một url
    ...
    def writeCsv(self, df: pd.DataFrame, path: str):
        try:
            df.to_csv(path)
        except:
            print('⚠ Error!')
...
class CFolder:
    def getAllFileInsideFolder(self, path: str, file_type: str):
        lst_files: List[str] = []
